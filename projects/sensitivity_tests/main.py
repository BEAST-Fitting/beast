...
def gen_summary_tables(gext_grid, keys, outdir, outname):
    ...
    if _outdir[-1] != '/':
        _outdir += '/'
    summary_table = expectation_values_pytables(gext_grid, keys=keys, filepath=_outdir + outname + '.hf5', method='maxprob')
...
def main():
    ...
    _fig = plot_keys(keys, outdir, outnames, show=False)
    _fig.savefig('mf02.png')
