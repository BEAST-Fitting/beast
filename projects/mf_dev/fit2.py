...
from beast.proba.likelihood import N_logLikelihood_NM
...
def tee(args, func_seq):
    ...
    This corresponds to a map call where the arguments are fixed and functions
    are the variable part.
...
def compute_sparse_lnp(obk, model_seds, ast_bias, ast_error, threshold=40, **kwargs):
    ...
    The likelihood itself will be N_logLikelihood_NM.
    ...
    ast_bias: ndarray, dtype=float, ndim=2
        noise model predicting flux biases per model

    ast_error: ndarray, dtype=float, ndim=2
        noise model predicting flux dispersion after biases per model
    ...
    .. note::
        why normal and not split normal?
        MF believes we have skewed distributions here.
    """
    (sed) = obk
    (lnp, chi2) = N_logLikelihood_NM(sed, model_seds, ast_error, ast_bias,
                                     mask=None, lnp_threshold=abs(threshold))

    indx = np.where((lnp - max(lnp[np.isfinite(lnp)])) > threshold)
    input_arr = np.array([sed]).T
...
def fit_model_seds_pytables(obs, sedgrid, ast, threshold=-40,
                            outname='lnp.hd5', gridbackend='cache', **kwargs):
    """
    ...
    -------
    outname: str
        file in which is stored the sparse structure
    """
    with tables.openFile(outname, 'w') as outfile:
        ...
        for r in fit_model_seds_memory(obs, g0, ast, threshold=threshold,
                                       gridbackend=gridbackend, **kwargs):

            input_arr, idx_arr, lnp_arr, model_seds = r
...
@generator
def fit_model_seds_memory(obs, sedgrid, ast, threshold=-40, gridbackend='cache', **kwargs):
    """
    Fit model seds with noise for sensitivity tests
    ...
    for tn, obk in Pbar(len(obs), desc='Calculating Lnp').iterover(obs.enumobs()):
        ...
        input_arr, idx_arr, lnp_arr, model_seds = compute_sparse_lnp(obk, _seds, ast_bias, ast_error, threshold)
