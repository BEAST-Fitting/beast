...
# Used to split the observed catalog and ASTs into source density bins for the BEAST
#
...
def make_source_dens_map(catfile, pix_size = 10., mag_cut = [24.5,27]):
    """
    ...
    ra_delt = dec_delt
    n_x = np.fix(np.round(math.cos(0.5*(max_dec+min_dec)*math.pi/180.)*(max_ra-min_ra)/ra_delt))
    ra_delt *= -1.
    ...
    for i in range(n_x):
        ...
        for j in range(n_y):
            ...
            n_indxs = len(indxs)
            #indxs_for_SD,= np.where((cat['f475w_vega'][indxs] >= mag_cut[0]) & (cat['f475w_vega'][indxs] <= mag_cut[1]))
    ...
    # Save to FITS file
    hdu.writeto(catfile.replace('.fits','_source_den_image.fits'),clobber=True)

    # Save the source density for individual stars in a new catalog file
    cat['SourceDensity'] = source_dens
    cat.write(catfile.replace('.fits','_with_source_density.fits'),overwrite=True)

if __name__ == '__main__':

    make_source_dens_map('HTTP_production/http_2015_09_14_fullflux_23sep15.fits')
