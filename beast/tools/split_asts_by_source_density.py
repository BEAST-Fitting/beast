...
def split_asts(ast_file, sd_map_file, bin_width=1.0):
    """
    ...
    for i in range(len(sd_bins) - 1):
        ...
        sd_ind = np.where(
            (sd_data["value"] >= sd_bins[i]) & (sd_data["value"] < sd_bins[i + 1])
        )[0]
        ...
        for j, ind in enumerate(sd_ind):
            ...
            ast_ind = np.where(
                (ast_data["RA_J2000"] > sd_data["min_ra"][ind])
                & (ast_data["RA_J2000"] < sd_data["max_ra"][ind])
                & (ast_data["DEC_J2000"] > sd_data["min_dec"][ind])
                & (ast_data["DEC_J2000"] < sd_data["max_dec"][ind])
            )[0]
        ...
        if len(ast_all_ind) > 0:
            ...
            new_filename = ast_file.replace(
                ".fits", "_SD" + str(sd_bins[i]) + "-" + str(sd_bins[i + 1]) + ".fits"
            )
