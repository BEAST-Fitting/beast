...
def write_slurm_file(slurm_filename, brick, log_path, joblist_file, queue_name='largemem', queue_ntasks='32'):
    ...
    sf.write('#\n')
    sf.write('#SBATCH -J BeastF'+str(brick)+'             # Job name\n')
...
if __name__ == '__main__':
    ...
    for i, cat_file in enumerate(cat_files):
        ...
        if sed_size[i] <= normal_queue_size:
            npf.write('./run_production_memory.py -f ' +ext_switch + brick + ' ' + sd_num + ' '+sub_num+' > '+log_path+'beast_fit_b'+brick+'_sd'+sd_num+'_sub'+sub_num+'.log\n')
        else:
            pf.write('./run_production_memory.py -f ' +ext_switch + brick + ' ' + sd_num + ' '+sub_num+' > '+log_path+'beast_fit_b'+brick+'_sd'+sd_num+'_sub'+sub_num+'.log\n')
