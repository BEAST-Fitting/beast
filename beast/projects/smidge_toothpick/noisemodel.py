...
class SMIDGE_ToothPick_Noisemodel(toothpick.MultiFilterASTs):
...
def make_splinter_noise_model(outname, sedgrid, frac_unc=0.10, absflux_a_matrix=None, **kwargs):
    """ splinter noise model assumes that every filter is independent with
    any other.  And assumes a fractional uncertainty at all fluxes.  No ASTs are used.
...
def make_toothpick_noise_model(outname, astfile, sedgrid, absflux_a_matrix=None, **kwargs):
    ...
    # read mag_in, mag_out
    model = SMIDGE_ToothPick_Noisemodel(astfile, sedgrid.filters)
