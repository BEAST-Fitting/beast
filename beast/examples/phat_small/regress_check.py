            ...
            if is_dataset:
                ...
                if all_names is None:
                    if hdfa[sname].dtype.isbuiltin:
                        if len(hdfa[sname].value) != len(hdfb[sname].value):
                    ...
                    for cname in all_names:
                        if cname in all_namesb:
                            if len(hdfa[sname][cname]) != len(hdfb[sname][cname]):
...
if __name__ == "__main__":
    ...
    hdf5_files = ["seds.grid", "seds_trim.grid", "noisemodel", "noisemodel_trim.grid"]
    for cfile in hdf5_files:
        file1 = basename + "/" + basename + "_" + cfile + ".hd5"
        if os.path.isfile(file1):
            hd = hdf5diff(file1, basename + "_good/" + basename + "_" + cfile + ".hd5")
