...
from beast_lh.run_beast.helper_functions import (subcatalog_fname,
                                                 parallel_wrapper,
...
def fitting(use_sd=True, nsubs=1, nprocs=1,
    ...
    if nsubs > 1:
        ...
        # -- using source density info
            ...
            phot_file_list = sorted( glob.glob(data.obsfile.replace('.fits','_SD*_sub*.fits')) )
        ...
        # -- no source density splitting
            ...
            for gridsub in gridsub_list:
                ...
                gridpickle_files.append('{0}/grid_info_dict.pkl'.format(datamodel.project) )
        ...
        for i in range(len(gridpickle_files)):
            if not os.path.isfile(gridpickle_files[i]):
                ...
                modelsedgrid_list = [x for x in modelsedgrid_files
                                         if os.path.dirname(gridpickle_files[i]) in x]
                noise_list = [x for x in noise_files
                                         if os.path.dirname(gridpickle_files[i]) in x]
                pdb.set_trace()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--nprocs", type=int, default=1,
                        ...
                        the subgrids''')
    parser.add_argument("--choose_sd_sub", type=str, default=None,
