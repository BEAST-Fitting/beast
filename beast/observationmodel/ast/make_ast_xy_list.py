...
def pick_positions_from_map(catalog, chosen_seds, input_map, input_column, N_bins, Npermodel,
...
def pick_positions(catalog, filename, separation, refimage=None, refcat=None):
    """
    ...
    refcat: string
        If provided, and refimage is set, the maximum values for x/y will
        be set by the maximum x/y of the catalog sources.  This avoids
        the selection of ASTs outside the image boundaries.
    ...
    print('original xmin/xmax: ', np.min(x_positions), np.max(x_positions))
    print('original ymin/ymax: ', np.min(y_positions), np.max(y_positions))
