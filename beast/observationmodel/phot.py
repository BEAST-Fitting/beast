...
Filter object information:
    ...
    definition contains {s.transmit.size:d} points""".format(
                s=self
            )
        )
...
def __load__(fname, ftab, interp=True, lamb=None, integrationFilter=False):
    ...
    if integrationFilter:
        ifT = numpy.interp(lamb, fname.wavelength, fname.transmit, left=0.0, right=0.0)
        ...
        if interp & (lamb is not None):
            ifT = numpy.interp(lamb, flamb, transmit, left=0.0, right=0.0)
...
def load_all_filters(interp=True, lamb=None, filterLib=None):
    ...
    with tables.open_file(filterLib, "r") as ftab:
        filters = [
            __load__(fname, ftab, interp=interp, lamb=lamb)
            for fname in ftab.root.content.cols.TABLENAME
        ]
    return filters
...
def fluxErrTomag(flux, fluxerr):
    ...
    """
    mag = fluxToMag(flux)
    return mag, -2.5 * numpy.log10(1.0 - fluxerr / flux)
...
def magErrToFlux(mag, err):
    ...
    """
    flux = magToFlux(mag)
    return flux, flux * (1.0 - magToFlux(err))
