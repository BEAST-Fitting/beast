...
def plot_ast_diagnostics(ast_file_list, seds_filename, interpolate=True, savefig=False):
    ...
    for n, nfile in enumerate(ast_file_list):
        ...
        print("Non-Zero Detections (%s): %i" %(model.filters[0].split("_")[-1]+'_VEGA', np.sum(model.data[model.filters[0].split("_")[-1]+'_VEGA'] != 99.999)))

        if interpolate == True:
            # interpolate all files with more than 100 detections
            if np.sum(model.data[model.filters[0].split("_")[-1]+'_VEGA'] != 99.999) > 100:
                bias, sigma, comp = model.interpolate(sedgrid)
                print("Interpolate : Successful")
                
            else: print("Interpolate : NOT Successful \n")
    ...
    if number_max > 1:
        ...
        # create colorbar
        cbar = fig1.colorbar(cm.ScalarMappable(cmap=cmaps), ax=ax.ravel().tolist(), ticks=np.arange(0, number_max, 1)/(number_max-1))
