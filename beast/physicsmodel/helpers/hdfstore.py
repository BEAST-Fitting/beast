...
def __createTable__(hd5, tab, group, tablename):
    ...
    t = hd5.create_table(
        group, tablename, desc, expectedrows=len(tab), createparents=True
    )
    hd5.flush()
    return t
    ...
    def set_mode(self, val=None, **kwargs):
        ...
        **kwargs is forwarded to tables.open_file
        ...
        if val is None:
            ...
            root_uep = kwargs.get("root_uep", None) or self._open_kwargs.get(
                "root_uep", "/"
            )
            filters = kwargs.get("filters", None) or self._open_kwargs.get(
                "filters", None
            )
            self._open_kwargs.update(
                dict(title=title, root_uep=root_uep, filters=filters)
            )
            if val != self._mode:
                self._mode = str(val)
        ...
        **kwargs
        ...
        with self as s:
            if s._mode[0] == "r":
                s.set_mode("a")
            __write__(
                data, s.source, group, tablename, append, silent, header, **kwargs
            )
    ...
    def __setitem__(self, key, value):
        ...
        with self as s:
            if key in list(s.keys()):
                raise KeyError(
                    "Object named {0} already exists in this store".format(key)
                )
            p = key.split("/")
            name = p[-1]
            p = "/".join(p[:-1])
