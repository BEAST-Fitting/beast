...
def _fits_writeto(
    ...
    hdu.writeto(
        filename, clobber=clobber, output_verify=output_verify, checksum=checksum
    )


def _fits_append(filename, data, header=None, checksum=False, verify=True, **kwargs):
    """
    ...
    name, closed, noexist_or_empty = pyfits.convenience._stat_filename_or_fileobj(
        filename
    )
...
def _ascii_read_header(
    fname, comments="#", delimiter=None, commentedHeader=True, *args, **kwargs
):
    """
...
def _ascii_generate_header(tab, comments="#", delimiter=" ", commentedHeader=True):
    """ Generate the corresponding ascii Header that contains all necessary info
...
def _latex_writeto(filename, tab, comments="%"):
    ...
    txt += tab.pprint(
        delim=" & ", fields="MAG*", headerChar="", endline="\\\\\n", all=True, ret=True
    )
    txt += "\\end{tabular}\n"
...
def __indent__(
    rows, header=None, units=None, headerChar="-", delim=" | ", endline="\n", **kwargs
):
    ...
    if headerChar not in (None, "", " "):
        rowSeparator = (
            headerChar * (sum(length) + len(delim) * (len(length) - 1)) + endline
        )
    else:
        rowSeparator = ""
...
class AstroHelpers(object):
    ...
    def hms2deg(_str, delim=":"):
        ...
        return neg * (
            (
                ((float(_str[-1]) / 60.0 + float(_str[1])) / 60.0 + float(_str[0]))
                / 24.0
                * 360.0
            )
        )
    ...
    def dms2deg(_str, delim=":"):
        ...
        if _str[0] == "-":
            ...
            neg = 1
        _str = _str.split(delim)
        return neg * ((float(_str[-1]) / 60.0 + float(_str[1])) / 60.0 + float(_str[0]))
    ...
    def sphdist(ra1, dec1, ra2, dec2):
        ...
        return 2.0 * rad2deg(
            arcsin(
                sqrt(
                    (sin((dec1_r - dec2_r) / 2)) ** 2
                    + cos(dec1_r) * cos(dec2_r) * (sin((deg2rad(ra1 - ra2)) / 2)) ** 2
                )
            )
        )
...
class SimpleTable(object):
    ...
    def __init__(self, fname, *args, **kwargs):
        ...
        if isinstance(fname, dict) or (dtype in [dict, "dict"]):
            ...
            if (extension == "csv") or dtype == "csv":
                ...
                n, header, units, comments, aliases, names = _ascii_read_header(
                    fname, commentedHeader=commentedHeader, **kwargs
                )
                ...
                if pyfits is None:
                    raise RuntimeError(
                        "Cannot read this format, Astropy or pyfits not found"
                    )
                if (
                    ("extname" not in kwargs)
                    and ("ext" not in kwargs)
                    and (len(args) == 0)
                ):
                    args = (1,)
                self.data = np.array(pyfits.getdata(fname, *args, **kwargs))
                header, aliases, units, comments = _fits_read_header(
                    pyfits.getheader(fname, *args, **kwargs)
                )
                self.header = header
                self._desc.update(**comments)
                self._units.update(**units)
                self._aliases.update(**aliases)
            elif (extension in ("hdf5", "hd5", "hdf")) or dtype in (
                extension in ("hdf5", "hd5", "hdf")
            ):
                if tables is None:
                    raise RuntimeError("Cannot read this format, pytables not found")
                hdr, aliases, units, desc, data = _hdf5_read_data(
                    fname, *args, **kwargs
                )
                self.data = data
                self.header = hdr
    ...
    def write(self, fname, **kwargs):
        ...
        if extension == "csv":
            ...
            np.savetxt(
                fname, self.data, delimiter=delimiter, header=hdr, comments="", **kwargs
            )
    ...
    def get(self, v, full_match=False):
        ...
        t._aliases.update(
            (k, v) for (k, v) in list(self._aliases.items()) if v in new_keys
        )
    ...
    def info(self):
        ...
        s = s.format(
            name=self.header.get("NAME", "Noname"),
            s=self,
        ...
        vals = [
            (k, self._units.get(k, ""), self._desc.get(k, "")) for k in self.colnames
        ]
        lengths = [
            (len(k), len(self._units.get(k, "")), len(self._desc.get(k, "")))
            for k in self.colnames
        ]
    ...
    def stack(self, r, defaults=None):
        ...
        self.data = recfunctions.stack_arrays(
            [self.data, r.data], defaults, usemask=False, asrecarray=True
        )
    ...
    def add_column(self, name, data, dtype=None, unit=None, description=None):
        ...
        if len(self.data.dtype) > 0:
            ...
            self.data = recfunctions.append_fields(
                self.data, name, _data, dtypes=dtype, usemask=False, asrecarray=True
            )
        else:
            if _data.ndim > 1:
    ...
    def stats(self, fn=None, fields=None, fill=None):
        ...
        fn = (stats.mean, stats.std, stats.min, stats.max, stats.has_nan)
...
class AstroTable(SimpleTable):
    ...
    def get_RA(self, degree=True):
        ...
        if (not degree) or (self.dtype[self._ra_name].kind != "S"):
            ...
            if len(str(self[0][self._ra_name]).split(":")) == 3:
                return np.asarray(AstroHelpers.hms2deg(self[self._ra_name], delim=":"))
    ...
    def get_DEC(self, degree=True):
        ...
        if (not degree) or (self.dtype[self._dec_name].kind != "S"):
            ...
            if len(str(self[0][self._dec_name]).split(":")) == 3:
                return np.asarray(AstroHelpers.dms2deg(self[self._dec_name], delim=":"))
    ...
    def where(self, condition=None, condvars=None, cone=None, zone=None, **kwargs):
        ...
        if zone is not None:
            if len(zone) != 4:
                raise ValueError(
                    "Expecting zone keywords as a tuple of 4 elements (ramin, ramax, decmin, decmax)"
                )
    ...
    def selectWhere(
        self, fields, condition=None, condvars=None, cone=None, zone=None, **kwargs
    ):
        """ Read table data fulfilling the given `condition`.
            Only the rows fulfilling the `condition` are included in the result.
