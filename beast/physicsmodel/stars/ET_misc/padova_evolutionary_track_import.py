...
data.meta['comments'] = ['Interpolated Padova evolutionary tracks',
...
eeps = np.array(([0] * 200) + [1] * 200 + [2] * 200 + [3] * 500 + [4] * 30 + [5] * 500 + [6] * 100 + [8] * 200)
...
for dir in directories:
    ...
    for file in files:
        ...
        # print(phase_col)
        phil_phase_array = Column(phase_col, name="evolutionary_point", description='rosenfield eep').T
...
data['mbolmag'] = Column(data['mbolmag'],
                         description='Bolomertric luminosities (magnitudes), calculated using: 4.77 - 2.5 * log L')
data['logg'] = Column(data['logg'], unit='cm/s**2',
                      description='surface gravity, calculated using:  -10.616 + log mass + 4.0 * log Teff - log L')
