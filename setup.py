...
PACKAGENAME = metadata.get('package_name', 'beast')
DESCRIPTION = metadata.get('description', 'Bayesian Extinction and Stellar Tool')
AUTHOR = metadata.get('author', 'Karl Gordon and BEAST Team')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'http://beast.readthedocs.io/')
__minimum_python_version__ = metadata.get("minimum_python_version", "2.7")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: beast requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
